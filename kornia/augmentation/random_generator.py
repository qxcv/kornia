...
def random_affine_generator(
    ...
    return _get_random_affine_params(
        batch_size, height, width, degrees_tmp, translate, scale, shear_tmp, resample, padding_mode, same_on_batch)
...
def _get_random_affine_params(
    ...
    resample: Union[str, int, Resample] = Resample.BILINEAR.name,
    padding_mode: Optional[str] = None, same_on_batch: bool = False
) -> Dict[str, torch.Tensor]:
